...
public class MyLinkedList<T> {
    ...
    /**
     * Remove por comteudo
     * @param content T sendo o objeto que deseja remover da lista
     * @return T sendo o objeto removido da lista ( nota-se que é o mesmo 
     * objeto passado como parametro )
    */
    public T removeByContent(T content){
        if(this.fist == null)
            return null;
        NodeList<T> temp1 = fist;// variavel que aponta para o no anterior ao que sera removido, caso nao primeiro.
        if (temp1.getContent() == content) {
            fist = fist.getNext();
            this.size--;
        ...
        if(temp2 == null)
            return null;
        while (temp2 != null && temp2.getContent() != content) {
            temp1 = temp1.getNext();
            temp2 = temp2.getNext();
        }
        if(temp2 == null)
    ...
    /**
    * Funçao que verifica se um objeto esta na lista ou nao
     * @param obj T sendo o objeto que deseja verificar se esta contido na lista
    * @return boolean sendo true para contein e false pana nao contain
    */
